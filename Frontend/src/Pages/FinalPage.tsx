...
const FinalPage = () => {
    ...
    // ── use result from store, fallback to mock while loading ──
    const data = result ?? {
        score: -19,
        teamSize: 12,
    ...
    return (
        // ... rest of your JSX unchanged — it already uses `data` correctly
            ...
            <div className="w-full max-w-[900px] flex flex-col gap-5">
                ...
                <div className="bg-white border border-[#e5e5e0] rounded-2xl p-8">
                    ...
                    <div className="flex gap-5 mb-6">
                        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-red-400" /><span className="text-xs text-[#888]">Current plan</span></div>
                        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-green-500" /><span className="text-xs text-[#888]">Suggested plan</span></div>
                    </div>
                        ...
                        {data.modelAnalysis.map((m: any) => (
                            <div key={m.name} className="bg-[#fcf5cc] border border-[#e5e5e0] rounded-2xl p-5">
                                ...
                                <div className="flex gap-6 items-end">
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-end gap-3">
                                            <div className="flex flex-col items-center gap-1">
                                                ...
                                                <div className="w-10 bg-red-400 rounded-t-lg" style={{ height: `${m.currentPerformance * 1.2}px` }} />
                                    ...
                                    <div className="w-px bg-[#e5e5e0] self-stretch" />
                                    <div className="flex-1 text-xs text-[#888] leading-relaxed self-center">{m.comparisonNote}</div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-[#fcf5cc] border border-[#e5e5e0] rounded-2xl p-8">
                    ...
                    <div className="flex flex-wrap gap-2 mt-5">
                        {data.recommendations.map((r: string) => (
                            <div key={r} className="bg-white border border-[#e5e5e0] rounded-full px-4 py-1.5 text-xs font-semibold text-[#555]">{r}</div>
                        ))}
                    </div>
                </div>
